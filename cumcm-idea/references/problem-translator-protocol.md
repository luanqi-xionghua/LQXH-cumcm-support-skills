...
> Imported into `cumcm-idea` as the Phase-1 protocol. The controller's combined output contract overrides this standalone protocol's instruction to create a separate translator report.
...
- [sentence-interpretation-rules.md](sentence-interpretation-rules.md)
- [historical-review-signals.md](historical-review-signals.md)
...
10. Write the audited Phase-1 material into the corresponding sections of the single `CUMCM_IDEA.md` document following `cumcm-idea-output-contract.md`. Do not export an intermediate translator report. Do not finalize the combined document if the semantic Mermaid flowchart is missing, invalid, or omits any numbered question.

## Required Phase-1 content

Map the following content into the Phase-1 sections defined by `cumcm-idea-output-contract.md`; that combined contract controls the final heading order.

### 协议内容 → 合同章节映射

| 本协议内容 | 输出合同章节 | 说明 |
|---|---|---|
| 输入范围与状态信息（控制器收集，本协议核验） | 第 0 节 | 源文件、附件覆盖、未决歧义与 HANDOFF_STATUS 由此处汇总 |
| 1. 整题概览 | 第 1 节 | 一一对应 |
| 2. 逐句题意翻译与联动表 | 第 2 节（逐句题意翻译与覆盖台账） | 合同版表格增加“来源页码/位置”列 |
| 3. 核心术语与口径表 | 第 3 节（仅题目术语） | 建模口径另见合同第 9 节，不得混入 |
| 4. 各问输入—任务—输出表 | 第 4 节 | 合同版表格增加“验收证据”列 |
| 5. 跨问题联动链 | 第 5 节（题意层跨问题联动链） | 全篇唯一语义联动图；Phase-2 禁止复制 |
| 6. 最容易漏读或误解的句子 | 第 6 节 | 一一对应 |
| 7. 完整性核验 | 第 7 节（Phase-1 完整性核验） | 一一对应 |
